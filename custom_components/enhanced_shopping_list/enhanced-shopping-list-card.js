...
class EnhancedShoppingListCard extends HTMLElement {
  ...
  _bindItemEvents(container, items, isCompleted) {
    container.querySelectorAll(".item-container").forEach((el) => {
      ...
      if (isCompleted) {
        el.querySelector(".completed-item").addEventListener("click", () => {
          this._toggleComplete(item);
        });
      }

      // Swipe delete button
      const deleteBtn = el.querySelector(".delete-btn-swipe");
      if (deleteBtn) {
  ...
  static get cardStyles() {
    return `
      ...
      .item-container {
        ...
        border-radius: var(--esl-radius); margin-bottom: 4px;
      }
      .swipe-bg-right {
        position: absolute; top:0; left:0; bottom:0; width:100%;
        background: #4caf50; display: flex; align-items: center;
        padding-left: 16px; color: white; font-weight: bold;
      }
      .swipe-bg-left {
        position: absolute; top:0; right:0; bottom:0; width:100%;
        background: #f44336; display: flex; align-items: center;
        justify-content: flex-end;
      }
      .swipe-icon { font-size: 20px; }
      .delete-btn-swipe {
        display: flex; align-items: center; justify-content: center;
        width: 80px; height: 100%; color: white;
      ...
      /* Note editor */
      .note-editor { padding: 0 8px 8px 38px; background: var(--card-background-color, #fff); }
      .note-input {
        width: 100%; box-sizing: border-box;
        padding: 6px 8px;
        border: 1px solid var(--divider-color, #e0e0e0);
